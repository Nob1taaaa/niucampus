...
const ClaimVerificationModal = ({ open, onOpenChange, post, userId, onClaimSubmitted }: ClaimVerificationModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md rounded-3xl border-primary/15 bg-card/95 backdrop-blur-xl max-h-[80dvh] overflow-y-auto [&_textarea]:scroll-mt-4" onOpenAutoFocus={(e) => e.preventDefault()}>
        <DialogHeader>
