...
const ListingDetail = ({ listing, user, onBack, onChat, onRefresh }: Props) => {
  ...
  const handleMarkSold = async () => {
    setMarking(true);
    const { error } = await (supabase as any).from("marketplace_listings").update({ is_sold: true }).eq("id", listing.id);
  ...
  const handleStartChat = async () => {
    ...
    if (!existing) {
      const { error } = await (supabase as any).from("marketplace_chats").insert({
    ...
    <main className="mx-auto max-w-3xl px-3 pb-16 pt-5 sm:px-4 sm:pt-6 md:px-6 md:pt-8">
      ...
      <div className="rounded-2xl border border-border/40 bg-card/70 backdrop-blur-sm overflow-hidden">
        {images.length > 0 ? (
          <div className="relative">
            <div className="aspect-[16/10] bg-muted/30">
              <img src={images[currentImage]?.image_url} alt={listing.title} className="h-full w-full object-contain bg-muted/10" />
            </div>
            {images.length > 1 && (
              <div className="flex gap-1.5 p-2 justify-center">
                {images.map((img, i) => (
                  <button key={img.id} onClick={() => setCurrentImage(i)} className={`h-12 w-12 rounded-lg overflow-hidden border-2 transition-all ${i === currentImage ? "border-primary" : "border-transparent opacity-60"}`}>
        ...
        <div className="p-4 sm:p-6 space-y-4">
          ...
          <p className="text-2xl font-extrabold text-primary">{listing.is_free ? "Free 🎁" : `₹${listing.price}`}</p>
          {listing.description && <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap">{listing.description}</p>}
          ...
          <div className="flex items-center gap-3 rounded-xl border border-border/40 bg-muted/10 p-3">
            ...
            <div>
              ...
              <p className="text-[0.65rem] text-muted-foreground">Posted {new Date(listing.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
            </div>
          </div>

          <div className="flex flex-wrap gap-2 pt-2">
            {isOwner ? (
              <Button onClick={handleMarkSold} disabled={marking || listing.is_sold} className="rounded-xl gap-1.5">
