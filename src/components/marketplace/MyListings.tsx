...
interface MyListing {
  id: string; title: string; price: number; is_free: boolean; is_urgent: boolean; is_sold: boolean; category: string; created_at: string;
}
...
const MyListings = ({ user, onRefresh }: Props) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (!error) setListings((data || []) as MyListing[]);
      setLoading(false);
    };
    load();
  }, [user]);
  ...
  const handleDelete = async (id: string) => {
    ...
    toast({ title: "Listing deleted" }); onRefresh();
  };

  const handleToggleSold = async (id: string, currentlySold: boolean) => {
    const { error } = await (supabase as any).from("marketplace_listings").update({ is_sold: !currentlySold }).eq("id", id);
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); return; }
    setListings(prev => prev.map(l => l.id === id ? { ...l, is_sold: !currentlySold } : l));
    toast({ title: currentlySold ? "Marked as available" : "Marked as sold! 🎉" }); onRefresh();
  };
      ...
      {listings.map(l => (
        <div key={l.id} className={`rounded-2xl border border-border/40 bg-card/70 backdrop-blur-sm p-3 sm:p-4 flex items-center gap-3 transition-all ${l.is_sold ? "opacity-60" : ""}`}>
          ...
          <div className="flex gap-1.5 shrink-0">
            <Button size="icon" variant="outline" className="h-8 w-8 rounded-xl border-primary/20 hover:bg-primary/5" onClick={() => handleToggleSold(l.id, l.is_sold)} title={l.is_sold ? "Mark available" : "Mark sold"}>
              <CheckCircle className={`h-3.5 w-3.5 ${l.is_sold ? "text-primary" : "text-muted-foreground"}`} />
            </Button>
            <Button size="icon" variant="outline" className="h-8 w-8 rounded-xl border-destructive/20 hover:bg-destructive/5" onClick={() => handleDelete(l.id)}>
